...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            $brand = new Brand($brandName, $slug, $imageName, $description, $status);
            if ($brandDAO->insert($brand)) {
            header("Location: index.php");
            exit();
            } else {
                $errors[] = "Thêm thương hiệu thất bại. Vui lòng thử lại.";
            }
        }
    }
}
...
<input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <div class="mb-3">
                ...
                <input type="text" name="brandName" class="form-control" value="<?= isset($_POST['brandName']) ? htmlspecialchars($_POST['brandName']) : '' ?>">
            </div>
            <div class="mb-3">
